...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Joshua Strozzi" };
...
const int LOCAL_SIZE    = 64;			//local work group
...
InitLists( )
{
	...
	// ****************************************
		...
		glColor3f( 1.f, 0.f, 0. );	// 0. <= r,g,b <= 1.
		glPushMatrix( );
			glTranslatef( -100.f, -800.f, 0.f );
			glutWireSphere( 600.f, 100, 100 );
		glPopMatrix( );

		glColor3f(0.f, 1.f, 0.);	// 0. <= r,g,b <= 1.
		glPushMatrix();
			glTranslatef(200.f, -2000.f, 0.f);
			glutWireSphere(1000.f, 100, 100);
		glPopMatrix();
