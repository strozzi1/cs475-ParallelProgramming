...
#pragma omp parallel for default(none) shared(xcs,ycs,rs,tn) reduction(+:numHits)
		for (int n = 0; n < NUMTRIALS; n++)
		{
			...
			if (d < 0)//If d is less than 0., then the circle was completely missed. (Case A) Continue on to the next trial in the for - loop.
				continue;
			...
			if (tmin < 0)//If tmin is less than 0., then the circle completely engulfs the laser pointer. (Case B) Continue on to the next trial in the for - loop.
				continue;
			...
			if (tt < 0)//If tt is less than 0., then the reflected beam went up instead of down.Continue on to the next trial in the for - loop.
				continue;
            else {
                numHits++;
                continue;
